package com.example.medprep.db.db;
...
public class ChatSQLiteDBHelper extends SQLiteOpenHelper {
    ...
    public static List<User> getAllUsersFromDB(Context context) {
        ...
        if (cursor.moveToFirst()) {
            do {
                @SuppressLint("Range") int retId = cursor.getInt(cursor.getColumnIndex(USER_COLUMN_ID));
                @SuppressLint("Range") String retName = cursor.getString(cursor.getColumnIndex(USER_COLUMN_NAME));
                @SuppressLint("Range") String retBirthDate = cursor.getString(cursor.getColumnIndex(USER_COLUMN_BIRTH_DATE));
                @SuppressLint("Range") String retGender = cursor.getString(cursor.getColumnIndex(USER_COLUMN_GENDER));
                try {
    ...
    public static List<Chat> getAllChatsForUserFromDB(Context context, int userId) {
        ...
        if (cursor.moveToFirst()) {
            do {
                try {
                    @SuppressLint("Range") int retId = cursor.getInt(cursor.getColumnIndex(CHATS_COLUMN_ID));
                    @SuppressLint("Range") String newestRequest = cursor.getString(cursor.getColumnIndex(CHATS_COLUMN_NEWEST_REQUEST));
                    @SuppressLint("Range") String previousDoctorQuestion = cursor.getString(cursor.getColumnIndex(CHATS_COLUMN_PREVIOUS_DOCTOR_QUESTION));
                    @SuppressLint("Range") String previousDoctorQuestionId = cursor.getString(cursor.getColumnIndex(CHATS_COLUMN_PREVIOUS_DOCTOR_QUESTION_ID));
                    @SuppressLint("Range") Date date = DB_DATE_MESSAGE_FORMAT.parse(cursor.getString(cursor.getColumnIndex(CHATS_COLUMN_DATETIME)));
                    @SuppressLint("Range") String conditionsArray = cursor.getString(cursor.getColumnIndex(CHATS_COLUMN_CONDITIONS_ARRAY));
    ...
    public static List<ChatMessage> getAllMessagesForChat(Context context, int chatId) {
        ...
        if (cursor.moveToFirst()) {
            do {
                @SuppressLint("Range") int retId = cursor.getInt(cursor.getColumnIndex(MESSAGES_COLUMN_MESSAGE_ID));
                @SuppressLint("Range") String text = cursor.getString(cursor.getColumnIndex(MESSAGES_COLUMN_MESSAGE));
                @SuppressLint("Range") boolean isUserMessage = cursor.getInt(cursor.getColumnIndex(MESSAGES_COLUMN_IS_USER_MESSAGE)) == 1;
    ...
    public static int getNextUserIdAvailable(Context context) {
        ...
        if (cursor.moveToFirst()) {
            @SuppressLint("Range") int retint = cursor.getInt(cursor.getColumnIndex(USER_COLUMN_ID)) + 1;
            database.close();
            cursor.close();
            return retint;
        }
    ...
    public static int getNextChatIdAvailable(Context context) {
        ...
        if (cursor.moveToFirst()) {
            @SuppressLint("Range") int retint = cursor.getInt(cursor.getColumnIndex(CHATS_COLUMN_ID)) + 1;
            database.close();
            cursor.close();
            return retint;
        }
    ...
    public static int getNextMessageIdAvailable(Context context, int chatId) {
        ...
        if (cursor.moveToFirst()) {
            @SuppressLint("Range") int retint = cursor.getInt(cursor.getColumnIndex(MESSAGES_COLUMN_MESSAGE_ID)) + 1;
            database.close();
            cursor.close();
            return retint;
        }
